        ...
        if($complete != "false") {
            foreach($stock_adjustment->getProductVariants() as $variant) {
                ...
                if($warehouse_inventory_id != false)
                    $em->getRepository('InventoryBundle:Warehouse')->updateWarehouseInventoryById($warehouse_inventory_id['id'], $variant->getQuantity());
                else {
                    $em->getRepository('InventoryBundle:Warehouse')->addWarehouseInventory($warehouse, $variant->getProductVariant(), $variant->getQuantity());
                }
            }
        }
